    ...
    <div class="container body">
      <div class="main_container">
        ...
        <div class="right_col" role="main">
			...
			<div class="col-sm-12">
				<h4 class="page-title">更新文章</h4>
			</div>
			<form id="articleForm">
				<div class="form-group col-md-6" style="padding: 0 10px 0 30px;">
					<input id="eId" name="eId" type="hidden" value="${requestScope.essay.eId}"/>
					<input id="eCreatetime" name="eCreatetime" type="hidden" value="${requestScope.essay.eCreatetime}"/>
					<input id="ePageview" name="ePageview" type="hidden" value="${requestScope.essay.ePageview}"/>
					<input id="ePromulgator" name="ePromulgator" type="hidden" value="${requestScope.essay.ePromulgator}"/>
					
					<input class="form-control" placeholder="请输入文章标题(必须)" id="eTitle" name="eTitle" required value="${requestScope.essay.eTitle}"/>
				</div>
				
				<div class="form-group col-md-6" style="padding: 0 10px 0 30px;">
					<input  id="tags" class="form-control" placeholder="请输入文章标签" name="tags" value="${requestScope.tags}"/>
				</div>
				<div class="form-group col-md-6" style="padding: 0 10px 0 30px;">
	                <select id="select"  class="form-control" data-placeholder="请选择分类...">
	                   <c:forEach items="${requestScope.classifys}" var="classify">
	                   		<c:if test="${classify.cId == requestScope.essay.cId}">
	                   			<option name="cId" value="${classify.cId}" selected="selected">${classify.cName}</option>
	                   		</c:if>
	                   		<c:if test="${classify.cId != requestScope.essay.cId}">
            	...
            	<div id="html-container" class="form-group col-md-12" style="padding:0 10px 0 30px" >
            		 <div class="editormd"  id="test-editormd">
					    <textarea class="editormd-markdown-textarea" name="eContent" id="editormd">${requestScope.essay.eContent}</textarea>
            	...
            	<div class="form-group col-md-3 col-sm-4" style="padding:0 10px 0 30px" >
					...
					<c:if test="${requestScope.essay.eIssay == '1'}">
						<div id="parentBtn" class="close1">
							<span id="fff" class="closeSpan" style="font-size:12px;color:#FFFFFF;margin:2px">开启</span>
						    <div id="childBtn" class="close2"></div>
						</div>
					</c:if>
           		</div>
           		 <div class="clearfix"></div>
            ...
            <div class="text-right">
                ...
                <button type="button" class="btn btn-primary waves-effect waves-light" onclick="update('0');">
                    保存文章
                </button>
                <button type="button" class="btn btn-warning waves-effect waves-light" onclick="update('1');">
                    存为草稿
                </button>
            </div>
			</form>
	...
	<script type="text/javascript">
			...
			var testEditor =null;
  			...
  			function update(state){
		        ...
		        var url = "${pageContext.request.contextPath}/admin/updateEssay.action";
		        var data = {
		        			"eId":eId,
		        			"cId":cId,
		        			"eTitle":eTitle,
